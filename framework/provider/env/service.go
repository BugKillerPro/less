...
// NewlessEnv 有一个参数，.env文件所在的目录
// example: NewlessEnv("/envfolder/") 会读取文件: /envfolder/.env
// .env的文件格式 FOO_ENV=BAR
func NewlessEnv(params ...interface{}) (interface{}, error) {
	if len(params) != 1 {
		return nil, errors.New("NewlessEnv param error")
	}
...
func (en *lessEnv) AppEnv() string {
	return en.Get("APP_ENV")
}

// IsExist 判断一个环境变量是否有被设置
func (en *lessEnv) IsExist(key string) bool {
	_, ok := en.maps[key]
	return ok
}

// Get 获取某个环境变量，如果没有设置，返回""
func (en *lessEnv) Get(key string) string {
...
func (en *lessEnv) All() map[string]string {
	return en.maps
}
