...
var providerListCommand = &cobra.Command{
    ...
    RunE: func(c *cobra.Command, args []string) error {
        ...
        lessContainer := container.(*framework.lessContainer)
        // 获取字符串凭证
        list := lessContainer.NameList()
        // 打印
...
var providerCreateCommand = &cobra.Command{
    ...
    RunE: func(c *cobra.Command, args []string) error {
        ...
        // 检查服务是否存在
        providers := container.(*framework.lessContainer).NameList()
...
import "github.com/BugKillerPro/less/framework"
